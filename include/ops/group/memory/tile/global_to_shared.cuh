...
template<int axis, bool assume_aligned, bool commit, ducks::st::all ST, ducks::gl::all GL, ducks::coord::tile COORD=coord<ST>>
__device__ static inline void load_async(ST &dst, const GL &src, const COORD &idx) {
    kittens::load_async<axis, assume_aligned, commit, ST, GL, COORD, GROUP_THREADS>(dst, src, idx);
}
template<ducks::st::all ST, ducks::gl::all GL, ducks::coord::tile COORD=coord<ST>> // default case
__device__ static inline void load_async(ST &dst, const GL &src, const COORD &idx) {
    kittens::load_async<2, false, true, ST, GL, COORD, GROUP_THREADS>(dst, src, idx);
}